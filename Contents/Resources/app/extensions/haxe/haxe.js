...
HaxeContext.prototype = {
	...
	,send: function(categorie,restoreCommandLine,retry,priority) {
		...
		return new Promise(function(accept,reject) {
			...
			var onData1 = onData;
			_g.client.sendAll(onData1,false,categorie,0,false);
		});
	}
		...
		if(this.client.isPatchAvailable) {
			return new Promise(function(accept,reject) {
				if(!(ds.document != null && ds.lastModification > ds.lastSave)) {
					...
					_g.patchFullDocument(ds).then(function(ds1) {
						accept(ds1);
					},function(ds2) {
						reject(ds2);
					});
					...
					if(pf != null) {
						...
						doSave = function(ds3) {
							...
							ds3.document.save().then(function(saved) {
								...
								if(__map_reserved[path1] != null) {
									tmp1 = _this3.getReserved(path1);
								} else {
									tmp1 = _this3.h[path1];
								}
								var pf1 = tmp1;
					...
					while(tmp.hasNext()) {
						...
						if(ds2.document != null && ds2.lastModification > ds2.lastSave) {
							if(ds2.lastSave >= ds2.saveStartAt) {
								...
								var document2 = ds2.document;
								if(document2 != null) {
									this.diagnostics["delete"](document2.uri);
								}
			...
			if(HxmlContext.reCheckOption.match(text)) {
				...
				if(prefix == "-" && name == "D") {
					if(HxmlContext.reDefineParam.match(param)) {
						...
						if(__map_reserved[defineName] != null) {
							tmp = _this.getReserved(defineName);
						} else {
							tmp = _this.h[defineName];
						}
						var define = tmp;
						if(define != null) {
							sHover = define.doc;
	...
	,provideCompletionItems: function(document,position,cancelToken) {
		...
		return new Promise(function(accept,reject) {
			...
			if(!makeCall) {
				...
				if(doMetaCompletion) {
					var _g1 = 0;
					var _g2 = _g.hxContext.client.metas;
			...
			var make_request = function() {
				...
				_g.hxContext.send("completion@2",true,1,10).then(function(m) {
					...
					if(!cancelToken.isCancellationRequested) {
						if(m1.severity == 2) {
							_g.hxContext.applyDiagnostics(m1);
						}
					}
					reject([]);
				});
			...
			var doRequest = function() {
				...
				if(isPatchAvailable) {
					if(isDirty) {
						_g.hxContext.patchFullDocument(ds1).then(function(ds2) {
							make_request();
						},function(ds3) {
							reject([]);
						});
					...
					changeDebouncer.whenDone(function() {
						...
						var ps = [];
						var _g11 = 0;
						var _g21 = _g.hxContext.getDirtyDocuments();
	...
	provideDefinition: function(document,position,cancelToken) {
		...
		return new Promise(function(accept,reject) {
			...
			var doRequest = function() {
				...
				if(isPatchAvailable) {
					if(isDirty) {
						_g.hxContext.patchFullDocument(ds).then(function(ds1) {
							make_request();
						},function(ds2) {
							reject(null);
						});
					...
					while(_g1 < _g2.length) {
						...
						if(_g3 != null) {
							...
							if(info1 == null && ctx != null && line != "") {
								var msg1 = [ctx.fileName,ctx.line == null?"null":"" + ctx.line," character " + ctx.column + " ",line].join(":");
								info1 = haxe_Info.decode(msg1,workingDir);
							}
							if(info1 != null) {
					...
					case 2:
						...
						while(_g12 < datas2.length) {
							...
							if(haxe_HaxeClient.reCheckMeta.match(data2)) {
								_g.metas.push({ prefix : haxe_HaxeClient.reCheckMeta.matched(1), name : haxe_HaxeClient.reCheckMeta.matched(2), doc : haxe_HaxeClient.reCheckMeta.matched(3)});
							}
						}
						break;
					case 3:
